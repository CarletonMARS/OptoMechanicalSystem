...
if __name__ == "__main__":
    ...
    Cal = -34500/82.10591482;
    Cal = 35075/73;
    Cal = 35/67;
    """
    """Cal is time(s)/Degree"""

    
    # 'Mode' is a switch
    #    = 0: the position of the probe is incremented by specified # degrees
    #    = 1: dynamic measurement setting
    #    = 2: static measurement setting; voltages are varied at fixed
    ...
    # Pull s2p data from the vna and write to file.
    WriteData(path, str(Angle), Freq)
    ...
    if Mode == 0:
        ...
        time.sleep(2);
        Increment = EndAng - StartAng;
        print("Moving to Angle: " + str(StartAng+Increment))
        MessageToArduino = str(Increment*Cal)
        arduino.write(MessageToArduino.encode())
        ...
        Cur = 0;
        # util.debug_messages = True
        # KM: set 1st arg to True, so that PNA class is in 'dummy mode' for testing.
        #     This means that the script can be run in the absence of a valid connection.
        # p = PNA(False, 16, False)
        
        v = VNA(False)
        v.connect(16)
        v.set_sweep_params(FreqSweepParams(8, 12, 801, -10, 1, []))
        Freq = np.linspace(8, 12, 801)
    
        
        # TODO: The following is omitted until we need to use the NI device.
        # # KM on July 22
        # csvDirPath = 'C:\\Users\\keiganmacdonell\\OneDrive - Carleton University\\MARS-MTSTest\\Code_&_Info_Far_Field_Measurements\\FarField_Code\\csvDir'
        # vReader = voltageReader(csvDirPath)
        
        arduino = serial.Serial('COM4',115200)
        time.sleep(2)
        
        count = 0;
        for Angle in range(StartAngle, EndAngle+Increment, Increment):
            
            WriteData(path, str(Angle), Freq)

            Cur = Cur + Increment*Cal/1000 
            print("Moving to Angle: " + str((Angle+Increment)/1000))
            MessageToArduino = str(Cur - round(Increment*Cal*count/1000))
            arduino.write(MessageToArduino.encode())
            msg = arduino.readline().strip()
            msg2 = msg.decode('ascii')
            print(msg2)
        ...
        v.set_sweep_params(FreqSweepParams(8, 12, 801, -10, 1, []))
        Freq = np.linspace(8, 12, 801)
        WriteData(path, 'staticMeas', Freq)
    ...
    #time.sleep(2)
    #Freq = np.linspace(0.05, 40.05, 801)
    #path = '.\\measurement';
    #for Angle in range(StartAngle, EndAngle+Increment, Increment):
    #    WriteData(path, str(Angle), Freq)
